...
class BIPLOWSTest(ComparisonTestFramework):
    ...
    def run_test(self):
        ...
        spendtx = self.create_transaction(self.nodes[0], self.coinbase_blocks[0], self.nodewitaddress, 1.0)
        ...
        spendtx = self.create_transaction(self.nodes[0], self.coinbase_blocks[1], self.nodewitaddress, 1.0)
        highSifyTx(spendtx)
        spendtx.rehash()
        try:
        ...
        spendtx = self.create_transaction(self.nodes[0], self.coinbase_blocks[2], self.nodewitaddress, 1.0)
        ...
        block.rehash()
        block.solve()
        assert_equal(self.nodes[0].submitblock(ToHex(block)), "rejected")


        print ("Test 5: HIGH_S witness transaction should not be accepted to mempool")
        spendwittx = self.create_witness_transaction(self.nodes[0], txid1, self.nodewitaddress, 0.9)
        ...
        block.rehash()
        block.solve()
        assert_equal(self.nodes[0].submitblock(bytes_to_hex_str(block.serialize(True))), "rejected")
    ...
    def create_raw_witness_transaction(self, node, txid, to_address, amount):
    ...
    def create_witness_transaction(self, node, txid, to_address, amount):
        inputs = [{ "txid" : txid, "vout" : 0}]
        outputs = { to_address : amount }
        rawtx = node.createrawtransaction(inputs, outputs)
        signresult = node.signrawtransaction(rawtx)
