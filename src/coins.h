...
public:
    ...
    CCoin(CTxOut&& outIn, int nHeightIn, bool fCoinBaseIn) : out(std::move(outIn)), fCoinBase(fCoinBaseIn), nHeight(nHeightIn) {}
    CCoin(const CTxOut& outIn, int nHeightIn, bool fCoinBaseIn) : out(outIn), fCoinBase(fCoinBaseIn),nHeight(nHeightIn) {}
...
struct CCoinsCacheEntry
{
    ...
    explicit CCoinsCacheEntry(CCoin&& coin_) : coins(std::move(coin_)), flags(0) {}
};
...
public:
    ...
    virtual bool GetValue(CCoin &coins) const = 0;
...
{
public:
    //! Retrieve the CCoin (unspent transaction output) for a given outpoint.
    virtual bool GetCoins(const COutPoint &outpoint, CCoin &coins) const;
...
protected:
    ...
    /* Cached dynamic memory usage for the inner CCoin objects. */
