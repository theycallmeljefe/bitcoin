...
bool CDerivingKeyStore::AddMeta(const CKeyID &key, const CKeyID &keyParent, uint32_t nIndex, const CChainCode &chaincode, int nDepth)
{
    LOCK(cs_KeyStore);
    if (keyParent != 0 || !chaincode.IsNull()) {
