...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    printf("InvalidChainFound:  current best=%s  height=%d  work=%s  date=%s\n",
      BlockHashStr(hashBestChain).c_str(), nBestHeight, nBestChainWork.ToString().c_str(),
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    printf("SetBestChain: new best=%s  height=%d  work=%s  tx=%lu  date=%s\n",
      BlockHashStr(hashBestChain).c_str(), nBestHeight, nBestChainWork.ToString().c_str(), (unsigned long)pindexNew->nChainTx,
      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", pindexBest->GetBlockTime()).c_str());
