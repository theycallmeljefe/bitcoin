...
bool CTransaction::AreInputsStandard(CCoinsView& mapInputs) const
{
    ...
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        ...
        if (!EvalScript(stack, vin[i].scriptSig, *this, i, true, 0))
            return false;
...
int CMerkleTx::SetMerkleBranch(const CBlock* pblock)
{
    ...
    else
    {
        ...
        // TODO: find block in case pblock==NULL
...
bool CTxMemPool::accept(CCoinsDB& txdb, CTransaction &tx, bool fCheckInputs,
...
bool CTransaction::AcceptToMemoryPool(CCoinsDB& txdb, bool fCheckInputs, bool* pfMissingInputs)
{
...
bool CMerkleTx::AcceptToMemoryPool(CCoinsDB& txdb, bool fCheckInputs)
{
    if (fClient)
    {
...
/* bool CWalletTx::AcceptWalletTransaction(CTxDB& txdb, bool fCheckInputs)
{

    {
        ...
        BOOST_FOREACH(CMerkleTx& tx, vtxPrev)
        {
            ...
            {
                uint256 hash = tx.GetHash();
                if (!mempool.exists(hash) && !txdb.ContainsCoins(hash))
...
bool CBlock::DisconnectBlock(CBlockIndex *pindex, CCoinsView &view)
{
    ...
    for (int i = vtx.size() - 1; i >= 0; i--) {
        ...
        if (outs.vout.size() != tx.vout.size())
            return error("DisconnectBlock() : wrong number of outputs? database corrupted");

        for (unsigned int j=0; j<tx.vout.size(); j++)
            if (!outs.IsAvailable(j))
                return error("DisconnectBlock() : some outputs still spent? database corrupted");
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = 0; j < tx.vin.size(); j++) {
                ...
                CCoins coins;
                if (!view.GetCoins(out.hash, coins)) { // this can fail if the prevout was already entirely spent
...
bool FindUndoPos(CChainDB &txdb, int nFile, CDiskBlockPos &pos, unsigned int nAddSize);
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("%d (blk%05u.dat:0x%lx)  %s  tx %d",
            pindex->nHeight,
...
// * if e==13, we only know the resulting number is not zero, so output 1 + 10*(n - 1) + 9
