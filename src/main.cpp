...
void static PruneOrphanBlocks()
{
    ...
    do {
        std::multimap<uint256, COrphanBlock*>::iterator it2 = mapOrphanBlocksByPrev.find(hash);
        if (it2 == mapOrphanBlocksByPrev.end())
            break;
        it = it2;
        hash = it->second->hashBlock;
    } while(1);
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("UpdatedTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f\n",
      chainActive.Tip()->GetBlockHash().ToString().c_str(), chainActive.Height(), log(chainActive.Tip()->nChainWork.getdouble())/log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()).c_str(),
...
bool static DisconnectTip(CValidationState &state) {
    ...
    int64_t nStart = GetTimeMicros();
    {
        ...
        if (!DisconnectBlock(block, state, pindexDelete, view))
            return error("DisconnectTip() : DisconnectBlock %s failed", pindexDelete->GetBlockHash().ToString().c_str());
        assert(view.Flush());
    }
    if (fBenchmark)
...
bool ActivateBestChain(CValidationState &state) {
    ...
    while (!fComplete) {
        ...
        if (chainMostWork.Tip() == NULL) return true;
...
{
    {
        ...
        // in flight for over two minutes.
        int64_t nNow = GetTimeMicros();
        if (!pto->fDisconnect && state.nBlocksInFlight && 
            state.nLastBlockReceive < nNow - 60*1000000 && 
            state.vBlocksInFlight.front().nTime < nNow - 120*100000) {
