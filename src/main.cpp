...
bool CTransaction::AcceptToMemoryPool(CTxDB *ptxdb, bool fCheckInputs, bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        bool fInvalid = false;
        if (!FetchInputs(ptxdb, mapUnused, false, false, mapInputs, fInvalid))
        {
            if (fInvalid)
        ...
        if (nFees < MIN_RELAY_TX_FEE)
        {
            ...
            CRITICAL_BLOCK(cs)
            {
                ...
                if (dFreeCount > GetArg("-limitfreerelay", 15)*10*1000 && !pblockstore->IsTransactionFromMe(*this))
...
bool CTransaction::AcceptToMemoryPool(bool fCheckInputs, bool* pfMissingInputs)
{
    ...
    if (pblockstore->HasFullBlocks())
        ptxdb = new CTxDB("r");
    return AcceptToMemoryPool(ptxdb, fCheckInputs, pfMissingInputs);
}
...
bool CBlockStore::NeedInv(const CInv* pinv)
{
    CRITICAL_BLOCK(cs_main)
    {
        ...
        case MSG_BLOCK:
            ...
            return !mapBlockIndex.count(pinv->hash) && !mapOrphanBlocks.count(pinv->hash);
        }
    }
