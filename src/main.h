...
public:
    ...
    bool WriteToDisk(CDiskBlockPos &pos)
    {
        ...
        fflush(fileout);
        static int nBlocksWithoutCommit = 0;
        if (!IsInitialBlockDownload() || ++nBlocksWithoutCommit >= 500) {
            nBlocksWithoutCommit = 0;
            FileCommit(fileout);
        }
...
public:
    ...
    CBigNum bnChainWork;
    unsigned int nChainTx; // when this gets near 4 billion, change to 64-bit type
    int nStatus;
    ...
    std::string ToString() const
    {
        return strprintf("CBlockIndex(pprev=%08x, pnext=%08x, nHeight=%d, merkle=%s, hashBlock=%s)",
            pprev, pnext, nHeight,
            hashMerkleRoot.ToString().substr(0,10).c_str(),
...
/** CCoinsView that adds a memory cache for transaction to another CCoinsView */
