...
public:
    ...
    bool fReserve;
    bool fUsed;
    CKeyDump(const CSecret &vchSecretIn, bool fCompressedIn)
    {
        Init();
        secret.SetSecret(vchSecretIn, fCompressedIn);
    }
    CKeyDump()
    {
        Init();
    ...
    CRITICAL_BLOCK(pwalletMain->cs_wallet)
    {
        ...
        for (set<CBitcoinAddress>::iterator it = setAddress.begin(); it != setAddress.end(); ++it)
        {
            ...
            if (pwalletMain->GetKey(*it, key))
            {
                ...
                mapDump[*it] = CKeyDump(key.GetSecret(fCompressed), fCompressed);
            }
        }
            ...
            if (coin->GetDepthInMainChain(pindex)>0)
            {
                for (int i=0; i < coin->vout.size(); i++) 
                {
                    ...
                    if (nCredit > 0)
                    {
                        ...
                        ExtractAddress(out.scriptPubKey, vchAddress);
                        if (mapDump.count(vchAddress) == 0)
                        {
                            CSecret vchSecretDummy;
                            mapDump[vchAddress] = CKeyDump(vchSecretDummy, false);
                        }
    ...
    CRITICAL_BLOCK(pwalletMain->cs_wallet)
    {
        ...
        if (valKeys.type() != null_type)
        {
            ...
            BOOST_FOREACH(const Value& valKey, arrKeys)
            {
                ...
                CKey key;
                bool fCompressed = false;
                if (!key.SetSecret(secret.GetSecret(fCompressed), fCompressed))
                    continue;
        ...
        if (nRescanHeight != pindexBest->nHeight && !setReserveKey.empty())
        {
            ...
            BOOST_FOREACH(const PAIRTYPE(uint256,CWalletTx)& item, pwalletMain->mapWallet)
            {
                ...
                {
                    CBitcoinAddress address;
                    if (ExtractAddress(txout.scriptPubKey, address) && pwalletMain->HaveKey(address))
                    {
