        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKLOCKTIMEVERIFY:
                {
                    ...
                    uint32_t nTxLockTime = checker.GetTransactionLockTime();
                    if (!(
                          (nTxLockTime <  LOCKTIME_THRESHOLD && nLockTime <  LOCKTIME_THRESHOLD) ||
                          (nTxLockTime >= LOCKTIME_THRESHOLD && nLockTime >= LOCKTIME_THRESHOLD)
                         ))
                    ...
                    if (nLockTime > (int64_t)nTxLockTime)
                        return set_error(serror, SCRIPT_ERR_UNSATISFIED_LOCKTIME);
...
bool SignatureChecker::IsFinalInput() const
{
    return txTo.vin[nIn].IsFinal();
}

uint32_t SignatureChecker::GetTransactionLockTime() const
{
