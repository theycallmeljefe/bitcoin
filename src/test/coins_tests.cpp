...
BOOST_AUTO_TEST_CASE(coins_cache_simulation_test)
{
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            BOOST_CHECK(entry.IsPruned() && coins.IsPruned() || entry == CCoin(coins.vout[0], coins.nHeight, coins.fCoinBase));
            ...
            if (coins.IsPruned()) {
                ...
                stack.back()->AddCoin(COutPoint(txid, 0), CCoin(coins.vout[0], coins.nHeight, coins.fCoinBase), true);
            }
        }
