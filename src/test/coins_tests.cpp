...
class CCoinsViewTest : public CCoinsView
{
    ...
    std::map<COutPoint, CCoin> map_;

public:
    bool GetCoins(const COutPoint& outpoint, CCoin& coin) const
    {
        std::map<COutPoint, CCoin>::const_iterator it = map_.find(outpoint);
        if (it == map_.end()) {
            return false;
        }
        coin = it->second;
...
//
// It will randomly create/update/delete CCoin entries to a tip of caches, with
...
BOOST_AUTO_TEST_CASE(coins_cache_simulation_test)
{
    ...
    std::map<COutPoint, CCoin> result;
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            CCoin& coin = result[COutPoint(txid, 0)];
            const CCoin& entry = stack.back()->AccessCoin(COutPoint(txid, 0));
            ...
            if (coin.IsPruned()) {
                ...
                stack.back()->AddCoin(COutPoint(txid, 0), CCoin(coin), true);
            }
        }
        ...
        if (insecure_rand() % 1000 == 1 || i == NUM_SIMULATION_ITERATIONS - 1) {
            for (auto it = result.begin(); it != result.end(); it++) {
                const CCoin& coin = stack.back()->AccessCoin(it->first);
...
typedef std::map<COutPoint, std::tuple<CTransaction,CTxUndo,CCoin>> UtxoData;
UtxoData utxoData;
...
BOOST_AUTO_TEST_CASE(updatecoins_simulation_test)
{
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            result[outpoint] = CCoin(tx.vout[0], height, CTransaction(tx).IsCoinBase());
        ...
        //1/20 times undo a previous transaction
            ...
            CCoin &origcoins = std::get<2>(utxod->second);
...
void SetCoinsValue(CAmount value, CCoin& coin)
{
    assert(value != ABSENT);
    coin.Clear();
...
void CheckAddCoinBase(CAmount base_value, CAmount cache_value, CAmount modify_value, CAmount expected_value, char cache_flags, char expected_flags, bool coinbase)
{
    ...
    try {
        ...
        test.cache.AddCoin(OUTPOINT, CCoin(std::move(output), 1, coinbase), coinbase);
        test.cache.SelfTest();
        GetCoinsMapEntry(test.cache.map(), result_value, result_flags);
