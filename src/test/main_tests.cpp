...
BOOST_AUTO_TEST_CASE(merkle_test)
{
    for (int i = 0; i < 32; i++) {
        ...
        for (int mutate = 0; mutate <= 3; mutate++) {
            ...
            uint256 unmutatedRoot = ComputeMerkleRoot(block, &unmutatedMutated);
            ...
            uint256 newRoot = ComputeMerkleRoot(block, &newMutated);
            ...
            if (mutate == 0) {
                for (int loop = 0; loop < std::min(ntx, 16); loop++) {
                    ...
                    std::vector<uint256> newBranch = ComputeMerkleBranch(block, mtx);
