...
class prevector_tester {
    ...
    void test() {
        ...
        for (Size s = 0; s < real_vector.size(); s++) {
            fprintf(stderr, "%x", real_vector[s] & 0xF);
        }
        for (Size s = real_vector.size(); s < real_vector.capacity(); s++) {
            fprintf(stderr, "-");
        }
        fprintf(stderr, "\nPre:  ");
        for (Size s = 0; s < pre_vector.size(); s++) {
            fprintf(stderr, "%x", pre_vector[s] & 0xF);
        }
        for (Size s = pre_vector.size(); s < pre_vector.capacity(); s++) {
            fprintf(stderr, "-");
        }
        fprintf(stderr, "\n");*/
        ...
        BOOST_FOREACH(const T& v, pre_vector) {
             BOOST_CHECK(v == real_vector[(&v) - &(pre_vector[0])]);
        }
