...
class TestConditionChecker : public Consensus::AbstractThresholdConditionChecker
{
public:
    int64_t BeginTime(const Consensus::Params& params) { return TestTime(10000); }
    int64_t EndTime(const Consensus::Params& params) { return TestTime(20000); }
    int Period(const Consensus::Params& params) { return 1000; }
    int Threshold(const Consensus::Params& params) { return 900; }
    bool Condition(const CBlockIndex* pindex, const Consensus::Params& params) { return (pindex->nVersion & 0x100); }

    void TestDefined(const CBlockIndex* pindexPrev, int num) { BOOST_CHECK_MESSAGE(GetStateFor(pindexPrev, paramsDummy) == Consensus::AbstractThresholdConditionChecker::DEFINED, strprintf("Test %i for DEFINED", num)); }
    void TestStarted(const CBlockIndex* pindexPrev, int num) { BOOST_CHECK_MESSAGE(GetStateFor(pindexPrev, paramsDummy) == Consensus::AbstractThresholdConditionChecker::STARTED, strprintf("Test %i for STARTED", num)); }
    void TestLockedIn(const CBlockIndex* pindexPrev, int num) { BOOST_CHECK_MESSAGE(GetStateFor(pindexPrev, paramsDummy) == Consensus::AbstractThresholdConditionChecker::LOCKEDIN, strprintf("Test %i for LOCKEDIN", num)); }
    void TestActive(const CBlockIndex* pindexPrev, int num) { BOOST_CHECK_MESSAGE(GetStateFor(pindexPrev, paramsDummy) == Consensus::AbstractThresholdConditionChecker::ACTIVE, strprintf("Test %i for ACTIVE", num)); }
    void TestFailed(const CBlockIndex* pindexPrev, int num) { BOOST_CHECK_MESSAGE(GetStateFor(pindexPrev, paramsDummy) == Consensus::AbstractThresholdConditionChecker::FAILED, strprintf("Test %i for FAILED", num)); }
};
...
public:
    ...
    VersionBitsTester& TestDefined() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                checker[i].TestDefined(vpblock.empty() ? NULL : vpblock.back(), num);
            }
        }
        num++;
        return *this;
    ...
    VersionBitsTester& TestStarted() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                checker[i].TestStarted(vpblock.empty() ? NULL : vpblock.back(), num);
            }
        }
        num++;
        return *this;
    ...
    VersionBitsTester& TestLockedIn() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                checker[i].TestLockedIn(vpblock.empty() ? NULL : vpblock.back(), num);
            }
        }
        num++;
        return *this;
    ...
    VersionBitsTester& TestActive() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                checker[i].TestActive(vpblock.empty() ? NULL : vpblock.back(), num);
            }
        }
        num++;
        return *this;
    ...
    VersionBitsTester& TestFailed() {
        for (int i = 0; i < CHECKERS; i++) {
            if ((insecure_rand() & ((1 << i) - 1)) == 0) {
                checker[i].TestFailed(vpblock.empty() ? NULL : vpblock.back(), num);
            }
        }
        num++;
        return *this;
...
BOOST_AUTO_TEST_CASE(versionbits_test)
{
    for (int i = 0; i < 64; i++) {
        ...
        // UNDEFINED -> DEFINED -> FAILED while threshold reached
                           .Reset().TestDefined()
                           .Mine(1, TestTime(1), 0).TestDefined()
        ...
        // UNDEFINED -> DEFINED -> LOCKEDIN at the last minute -> ACTIVE
                           .Reset().TestDefined()
                           .Mine(1, TestTime(1), 0).TestDefined()
                           .Mine(1000, TestTime(10000) - 1, 0x101).TestDefined() // One second more and it would be defined
                           .Mine(2000, TestTime(10000), 0x101).TestStarted() // So that's what happens the next period
