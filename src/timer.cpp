...
class Timer {
    ...
    // @pre: lock on mutex is acquired
    // @pre: job is running
    void Done_(TimerJob *job, boost::unique_lock<boost::mutex> &lock) {
...
public:
    // Construct a timer.
    Timer() : fExit(false), fRunning(false) {}
...
void static TimerThread() {
    timer.Run();
}

void StartTimer() {
    boost::thread t(TimerThread);
}
...
void TimerJob::Schedule(const boost::posix_time::time_duration &dur) {
...
TimerJob::TimerJob(bool autodelete) : 
    _time(boost::date_time::not_a_date_time), _running(false),
    _scheduled(false), _autodelete(autodelete) {}

TimerJob::TimerJob(const boost::system_time &time, bool autodelete) : 
    _time(boost::date_time::not_a_date_time), _running(false),
    _scheduled(false), _autodelete(autodelete) {
    Schedule(time);
}
