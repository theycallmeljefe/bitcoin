...
bool CCoinsViewDB::GetCoins(const COutPoint &txid, CCoin &coin) const {
    return db.Read(CoinsEntry(&txid), coin);
}
...
bool CCoinsViewDBCursor::GetValue(CCoin &coin) const
{
        ...
        if (pcursor->GetKey(key) && key.first == DB_COINS) {
            ...
            for (size_t i = 0; i < oldcoins.vout.size(); ++i) {
                if (!oldcoins.vout[i].IsNull() && !oldcoins.vout[i].scriptPubKey.IsUnspendable()) {
                    CCoin newcoin(std::move(oldcoins.vout[i]), oldcoins.nHeight, oldcoins.fCoinBase);
                    outpoint.n = i;
